...
shipped with 0 migrations.

**Why:** a dead global setting with a live-looking name is how the engine came to read the wrong one for two
months. **Notes:** a contract migration (`DELETE FROM app_settings WHERE key = 'invoice_run_day'`), on its own
`db/…` branch, with a rollback file. Grep `supabase/tests` and `seed.sql` first. Leave applied migrations alone.
...
### The "running low" fields save on every keystroke — **S** `[from the package-settings move 2026-09-24]`
Packages → *When is a package running low?* writes `tenants.low_package_lessons` / `package_expiry_warning_days`
on every `onChange` (kept from the Students page it moved from). Typing "20" sends two PATCHes, "2" then "20".

**Why:** two requests can land out of order and store "2" while the field shows "20" — and that number decides
which families Generate renewal offers picks up. **Notes:** save on blur or after a short idle; keep
`parseCount`'s empty-is-not-zero rule (§7.22) and the disabled-until-loaded fields.
