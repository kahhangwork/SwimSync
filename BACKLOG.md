...
### Drop the dead `app_settings.auto_invoice_enabled` row — **S** `[raised 2026-09-25]`
The global on/off switch was superseded by `tenants.auto_invoice_enabled` when the engine became per-tenant
(`20260718000600_tenant_backfill.sql` copied it across). A 2026-09-25 grep found no reader in either app, the
engine source, `supabase/tests`, `supabase/cloud` or any SQL function body — the same state
`invoice_run_day`'s row was in before `20260925000100` dropped it.

**Why:** the same trap — a dead global switch with a live-looking name, beside a live per-tenant one of the
same name. **Notes:** before writing, `supabase functions download generate-invoices` and grep the DEPLOYED
source for `app_settings` (not done for this key yet). Then copy `20260925000100`'s shape: a contract migration
(`DELETE FROM app_settings WHERE key = 'auto_invoice_enabled'`) on a `db/…` branch, a rollback that re-inserts
`'true'::jsonb`, and one more `is_empty` in `app_settings_dead_keys.test.sql`. Leave applied migrations alone.
