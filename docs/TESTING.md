...
| `partial_payment.test.sql` (17, §8.83) | partial-payment via `debit_balance` (`20260822000100`, §7.206): `apply_credit_to_invoice` folds a pure debit (`net = gross + debit`), NETS credit against debit (50 credit + 30 debit on a 20 invoice → net 0, both cleared), idempotency (a second apply never re-folds / drives `debit_balance` negative), re-correcting a voided-and-debited note is refused (`CN002`), the `debit_balance ≥ 0` CHECK, and the outstanding-invoice identity `net = gross − package − credit + adjustment`. Also `core.test.ts` +2 (engine folds a debit onto the next invoice; credit+debit net) and `email.test.ts` +2 (the +Adjustment line renders and the total reconciles) |
