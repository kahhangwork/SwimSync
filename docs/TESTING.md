...
| `package_weeks_start_date.test.sql` (17) | weeks/start-date phase A (`20260814000400`/`000500`): the months⇆weeks derive trigger, weeks-based expiry (`start + validity_weeks*7`), `suggest_package_start` = coverage-end + 1, and the **⚠ RISK 1 pins** — a parent cannot edit `validity_weeks`/`ph_extension_weeks`/`manual_extension_days`/ack columns, nor `start_date` on a pending OR active package (§7.157); an admin cannot move `start_date` once active; the active-row CHECK forbids a NULL `start_date` |
