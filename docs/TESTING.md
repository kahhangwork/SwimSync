...
| `holiday_day_rpc.test.sql` (8) | `mark_day_holiday`/`unmark_day_holiday` (`20260818000900`): a non-admin is refused, a non-calendar date is refused, voiding materializes a `'holiday'` row for **enrolled + trial guest** (expected-set parity), the covering package extends, re-voiding is idempotent, and unmark retracts the extension AND deletes the emptied session |
