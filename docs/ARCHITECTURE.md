...
| `.claude/skills/run-ui-playwright/drivers/check-driver-ports.sh` | CI guard: no driver hardcodes an app port (3xxx/80xx) — import `ADMIN`/`EXPO` from `lib.mjs` so a worktree drives its own build |
...
| `SwimSyncApp/lib/publicRoutes.ts` | The auth gate's session-less allowances, two lists: **public pages** (`/welcome`, `/invoice`, `/package` — any session, signed-in stays; matched on a SEGMENT boundary, so `/invoices` is gated) and **signed-out screens** (`/register`, `/forgot-password` — exact match, signed-in still redirected). `/reset-password` + `/accept-invite` deliberately OUT (token session). ⚠ widening either widens the gate |
...
| `SwimSyncAdmin/app/(admin)/platform/domain/moveStudentWarning.ts` | `totalFamilyCredit` — sums EVERY linked parent's credit at a business for the pre-move advisory (a child can have two parents; a one-parent read under-warns) |
...
| `SwimSyncApp/features/coach-classes/domain/weekOrder.ts` (moved from `lib/` 2026-09-23) | Groups the coach's classes by weekday with **today first**. The weekday is a **parameter**, never read from a clock inside — the same shape `timeOfDay.ts` forced after §7.7. Also pins `WEEK_ORDER` to the Postgres `day_of_week` enum's declaration order, which is why `.order("day_of_week")` sorts in week order rather than alphabetically. An unrecognised weekday is **kept and sorted last, never dropped** — losing a class silently is worse than showing it oddly |
...
| `SwimSyncAdmin/app/(admin)/trials/domain/trialConvert.ts`, `…/attendance/domain/makeupFromAttendance.ts`,  `SwimSyncAdmin/app/(admin)/history/domain/auditDiff.ts` | Wave C pure helpers (each `+.test`; the first two moved beside their sole importer by the tier refactor): the convert-a-trial two-press guard (§7.180); the make-up own-enrolled-class gate + host-choice exclusion; the Change History snapshot-diff + the "unresolved actor → *unknown user*, never *system*" label |
...
| `SwimSyncApp/features/parent-attendance/domain/upcomingLessons.ts` (+ test; moved from `lib/` 2026-09-23) | Parent upcoming-lessons projection: weekday walk over ~4 weeks from active enrolments, `tenant_public_holidays` subtracted. Derived at read time — no pre-generated sessions |
...
| `SwimSyncAdmin/app/(admin)/classes/domain/classRoster.ts` | Who is in a class: active enrolments + **upcoming** trials, and the `2+1` count. **Takes `today` as a required parameter and touches no clock** — §7.7 made unreachable rather than discouraged |
...
| `SwimSyncAdmin/app/(admin)/credit-notes/domain/creditNoteEmailState.ts` | The Credit Notes page's Resend gate; mirrors `is_tenant_admin()` term for term |
...
| `SwimSyncAdmin/app/(admin)/makeups/` · `lib/makeupSearch.ts` | **Feature-tiered 2026-09-18 (Admin L-D)** — `domain/useMakeups` holds every hook incl. the sort (§7.249), `domain/makeupRows` the pure host/date/expiry logic. Admin: book/cancel make-ups — child found via ONE search box matching name OR class title (pure filter in the lib, unit-tested), past-and-unmarked list, package-expiry advisory |
...
| `SwimSyncApp/app/invoice/[token].tsx` | The public invoice page (chrome-less, a public page in `lib/publicRoutes.ts`): computed QR, Save-QR-image + scan-from-gallery instruction, claim button |
...
| `SwimSyncAdmin/app/(admin)/levels/` | **Feature-tiered 2026-09-18 (Admin L-D)**; the tenant is resolved inside each insert in `domain/useLevels`. The business's level ladder **and** each rung's skill list (expand a row). Order is set here and preserved everywhere |
...
(`features/change-password/ui/ChangePasswordScreen.tsx` — moved from `components/` 2026-09-23 —, routes `…/settings/change-password.tsx` and
