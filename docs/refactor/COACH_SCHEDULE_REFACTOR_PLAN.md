...
| 2 | `f3b4e8a` | 12 / **11** | 488 → 505 | **Baseline net (Stage 1 code) = 17/17 GREEN:** schedule-week 21 · unmarked-lessons 12 · cancel-lesson 17 · coach-roster 30 · stale-screen 22 · tz-saturday 6 · trials 16 · bulk-setall 10 · smoke-app **73/73** · attendance-guard 22 · makeups 15 · trial-onboarding **10/10** · trial-visibility 11 · levels 9 · level-skills 14 · student-identity 13 · admin-lesson-detail 27 (Expo WITHOUT `CI=1`). **After Stage 2** (bundle proven to carry `buildSchedule`): schedule-week 21 · unmarked-lessons 12 · cancel-lesson 17 · coach-roster 30 · tz-saturday 6 · trials 16 = baseline. Every logic line of `scheduleIndex.ts` / `scheduleRows.ts` exists in the pre-cut route (script; only headers, signatures, returns and two `const X =` → `return`). Cases 1–10 incl. **3a/3b**, the null-roster leg, fixtures in `+08:00`. **Proven able to fail** (each mutation, reverted): backlog range narrowed to the week → 3b/4/5/6 red; only owners nagged → 3b; probe list deduped → 7; not-ended guard dropped → 4; probe widened to `showsWholeSchedule` → 3a (after adding a session to 3a — it first passed). `isTruncated` takes `rosterRes` (not `rosterRaw`) so its body stays verbatim. Route import block rebuilt from usage (14 moved symbols gone, `attendanceCompleteness` pin deleted). Route 1,182 → 923 |
| 3 | `bc331f7` | **0** / 7 | 505 | **Full net = 17/17 baseline** (same counts as the Stage 1 baseline; bundle proven to carry `fetchShadowAssignments`). Script (`check3.py`): **9 of 9** dao chains byte-identical to the original incl. terminals; exactly **7** token checks, in order, none after `sessionsRes`; `setTruncated` after the 6th check and before the probe; no `useMemo` / clock read in the hook code; exactly one `useRef(`; deps byte-identical; no effect in the hook. Temp `[sched] load` log: mount **1**, week-next **1**, Classes→Schedule **1**, 70 s idle **0**; removed. **Hand-check "before" run** (throwaway fixture on top of `fixtures-coach-roster.sql`, 9/9, screenshots `hc-before-*`): sub → Covering + Mark Attendance; shadow → Shadowing + View lesson; owner → Covered + View lesson; chips + filter; clamp (classes deactivated → chips gone, Sat 26 Sept visible unfiltered); DONE tap → `/classes/<id>/attendance?date=2026-09-16&from=schedule`. 16 pins left the ledgers (the 12 check-3 + store, supabase, markableFloor, sessionMainCoach). `bounds` stays on the route until Stage 4. Route 923 → 631 |
| 4 | `09e19d4` | 0 / 5 | 505 | schedule-week 21/21 · unmarked-lessons 12/12 (bundle carries `useScheduleSections`). Every body line of `useScheduleSections.ts` exists in the original (script; only header, imports, signature, return and the `bounds` comment differ). **Location hand-check 9/9** (the `hc-s4-*` screenshots): chips, filter, and the clamp — the only reachable clamp is a location's classes disappearing between loads (weekly recurrence means week navigation never empties one), done by deactivating them in the DB and refocusing. Route 631 → 574 |
| 5 | (this commit) | **0 / 0** | 505 | 13 `ui/` files (3 module-scope components whole + 10 render blocks), JSX moved verbatim with props destructured on line 1. **Ordered JSX text/literal sequence (incl. every className, testID, copy) identical: 141 = 141** (`check5.py`, proven able to fail: a COMING UP/DONE swap and a merged `DaySection` header each went red). Prose-prefix grep: nothing. **Tailwind** after `expo start --clear`, COMING UP day expanded by TAP: `pl-6` = **24px**, visible; control `text-sm` = 14px (`s5-tailwind.png`). **Hand-check "after" 9/9, identical to "before"** (role badges + View lesson, chips + filter + clamp, DONE tap URL). Route **105 → 101** lines after tidying (header comment, collapsed imports); **0** `useState`/`useRef`/`useMemo`; both ledgers EMPTY, `F_SCHED` deleted. Full net: see L4 |
| L4 | (this commit) | — | 505 | **Full net on the finished screen = baseline on all 17:** schedule-week 21 · unmarked-lessons 12 · cancel-lesson 17 · coach-roster 30 · stale-screen 22 · tz-saturday 6 · trials 16 · bulk-setall 10 · smoke-app 73 · attendance-guard 22 · makeups 15 · trial-onboarding 10 · trial-visibility 11 · levels 9 · level-skills 14 · student-identity 13 · admin-lesson-detail 27. Every Stage 0b–5 check above holds on this code |

**The route:** 1,255 → **101** lines; 8 `useState` + 1 `useRef` + 1 `useMemo` → **0**; 11 `@/lib` + `@/store` → **0**; the
9 client reads → `dao/`; both ledgers EMPTY. **The last full-track giant is done** — every giant in both apps (§7 of the
playbook) has now taken the full track.
...
- **BACKLOG — four `check()`s no driver has (all hand-checked at Stages 3–5, `hc-before-*` / `hc-after-*`):** the
  role badges on the coach Schedule (`Covering` / `Shadowing` / `Covered`) with `View lesson` vs `Mark Attendance`;
  the location chips + filter + clamp; and a tap on a DONE row landing on `…/attendance?date=…&from=schedule`. The
  throwaway `zz-sched-handcheck.mjs` + `hc-fixture.sql` (on top of `fixtures-coach-roster.sql`) are the spec. **The
  clamp is reachable ONLY when a location's classes vanish between loads** (weekly recurrence means week navigation
  never empties a location) — the chip-clamp comment's "when the selected location has no lessons this week" is the
  deactivation case in practice.
- **Playbook §4 / §7.254 evidence (Stage 5):** a regex click on a day heading hit a NEEDS MARKING row (their dates
  share the shape) and a `getByText(title).last()` hit a TODAY-card title from a screen mounted underneath. Target
  the DONE card by its own class (`div.text-sm.font-bold`) and a day by its EXACT label.
- **Playbook §2 mechanics (new, worth keeping):** rebuild the route's import block from SYMBOL USAGE at every stage
  (a table of module → names, emit only what the body references, exclude locally declared components) — no stale
  import survives a stage, and the fence's shrink test then names exactly the pins to delete. 23 pins left over
  Stages 2–5 this way, none by hand.
- **A characterisation test can pass a mutation for want of a fixture row** (Stage 2, case 3a): widening the probe to
  `showsWholeSchedule` passed until the case gained a session row. Mutate each guarded line once; a green run
  proves nothing until something has gone red.
