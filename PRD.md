...
**Exactly one admin per business.** That is a deferred decision rather than a designed
limit: until multiple admins are built, two people sharing the work share one login.
...
and lies **between the 1st of last month and today**. Both bounds are enforced by the
**database**, for every caller, not by the screen:
...
| **Tenancy** | *(implemented)* **Multi-tenant.** A tenant is a business; a private coach is a tenant of one. `superadmin` split into **tenant admin** (one business) and **platform admin** (cross-tenant support). No rule branches on private-vs-school. **A business is created by the platform admin** (§4.4) — one admin each, no public signup |
