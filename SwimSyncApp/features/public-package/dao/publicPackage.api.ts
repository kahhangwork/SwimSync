// The public-package Edge Function — the page's only transport
// (docs/refactor/BATCH_FGH_PLAN.md, App L-G). Both calls are byte-identical to the
// fetch( they replaced in app/package/[token].tsx and return the raw Response
...
//     (supabase/functions/public-package/index.ts), so any added header fails the
//     CORS preflight — and a failed fetch renders "Package not found", which is
...
export const fetchPublicPackage = (token: string | undefined) =>
  fetch(
    `${FUNCTIONS_URL}?token=${encodeURIComponent(token ?? "")}`,
  );

export const postPublicPackageClaim = (token: string | undefined) =>
  fetch(FUNCTIONS_URL, {
    method: "POST",
