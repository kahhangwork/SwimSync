-- ============================================================
-- ROLLBACK for 20260807000100_sgt_class_terms.sql.
--
-- Puts set_class_terms() and sync_class_display_price() back on CURRENT_DATE.
--
-- ⚠ WHAT ROLLING BACK RESTORES IS A LIVE BUG, so do it only to unblock
-- something worse. CURRENT_DATE is the date in the SESSION's time zone and this
-- database runs in UTC, while the admin panel sends todayInSg() — so between
-- 00:00 and 08:00 SGT every class edit is refused with "terms cannot start in
-- the future" and nothing saves. That is what production did for three weeks.
-- The nightly driver sweep (04:00 SGT) will go red again the same night.
--
-- Bodies below are VERBATIM the pre-20260807000100 definitions.
--
-- Verify afterwards (expect exactly these two, which is the BROKEN state):
...
BEGIN
  SELECT count(*) INTO v_n
   ...
   WHERE n.nspname = 'public' AND p.prosrc ~ 'CURRENT_DATE';
  IF v_n <> 2 THEN
    RAISE EXCEPTION 'expected exactly 2 functions back on CURRENT_DATE, found %', v_n;
  END IF;
END $$;
