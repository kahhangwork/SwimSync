...
VALUES
  ('00000000-0000-0000-0000-000000000000','a0000000-0000-0000-0000-0000000000c2',
   ...
   now(), '{"provider":"email"}','{"full_name":"Con Coach","role":"coach"}', now(), now(), '', '', '', ''),
...
VALUES ('c0000000-0000-0000-0000-000000000002','Con Kid','assigned');
...
SELECT 'e0000000-0000-0000-0000-000000000002', p.id, '2026-01', 30, 0, 30, 'outstanding'
...
INSERT INTO credit_notes (id, reference_number, parent_id, student_id, invoice_id, invoice_item_id,
  lesson_session_id, amount, original_status, corrected_status, status)
SELECT '0c000000-0000-0000-0000-000000000001','CN-TEST-0001', p.id,
...
SELECT throws_ok($$
  INSERT INTO invoices (parent_id, billing_month, gross_amount, credit_applied, net_amount, status)
  SELECT p.id, '2026-01', 99, 0, 99, 'outstanding' FROM parents p WHERE p.profile_id='a0000000-0000-0000-0000-0000000000d2'
