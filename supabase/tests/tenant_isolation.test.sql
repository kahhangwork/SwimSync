...
SELECT '30000000-0000-0000-0000-0000000000a1', c.id, 'Class A', 'saturday','10:00','11:00',(SELECT l.id FROM locations l WHERE l.tenant_id = c.tenant_id AND lower(trim(l.name)) = 'default location'), 25,
...
INSERT INTO classes (id, coach_id, title, day_of_week, start_time, end_time, location_id, price_per_lesson, category_id)
SELECT '30000000-0000-0000-0000-0000000000b1', c.id, 'Class B', 'sunday','10:00','11:00',(SELECT l.id FROM locations l WHERE l.tenant_id = c.tenant_id AND lower(trim(l.name)) = 'default location'), 40,
       (SELECT cc.id FROM class_categories cc
         WHERE cc.tenant_id = c.tenant_id
           AND lower(trim(cc.name)) = 'default group')
