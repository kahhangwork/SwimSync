// The admin-side twin of SwimSyncApp/lib/skillProgress.test.ts. The SOURCE file
// is pinned byte-identical by skillProgress.drift.test.ts; these test files are
// deliberately NOT pinned, because they must differ here — the test API is
// imported explicitly so the file stays tsc-clean for `next build`, which
// type-checks **/*.tsx (see vitest.config.ts).
