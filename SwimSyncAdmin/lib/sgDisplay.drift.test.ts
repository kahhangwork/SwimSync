...
// TWIN FILE: SwimSyncApp/lib/sgDisplay.drift.test.ts is identical except that it
...
const ALLOWED: { file: string; contains: string; why: string }[] = [
  ...
  {
    file: "SwimSyncApp/features/invoice-detail/domain/invoiceDetailFormat.ts",
    contains: "parseInt(month) - 1",
    why: "billing month from parts, month+year only",
  },
  {
    file: "SwimSyncApp/features/paynow/domain/usePayNow.ts",
    contains: "parseInt(month) - 1",
    why: "billing month from parts, month+year only",
  },
  {
    file: "SwimSyncApp/features/public-invoice/domain/monthLabel.ts",
